...
def _seed_points() -> list[tuple[float, float, float, float]]:
    return [
        (450.0, 1.000, 0.020, 0.010),
        (650.0, 0.784, 0.021, 0.011),
        (850.0, 0.593, 0.023, 0.012),
        (1050.0, 0.431, 0.026, 0.013),
        (1250.0, 0.305, 0.030, 0.015),
    ]


def _real_points() -> tuple[str, str, str, list[tuple[float, float, float, float]]]:
...
def _ingest(
    *,
    ...
    try:
        ...
        for x_value, measured, stat_err, sys_err in points:
            upsert_observation(
                conn,
                ObservationRecord(
...
def main() -> None:
    if want_real_ingest():
        try:
            ...
            print(f"[warn] hepdata real ingest failed; collider channel skipped ({exc})")
            return
