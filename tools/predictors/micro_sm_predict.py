...
SM_BASELINES: dict[str, float] = {
    "nu_theta23": 0.56,
    "nu_dm2_32": 2.50e-3,
    "nu_theta13": 0.0221,
    "nu_reactor_sin2_2theta13": 0.0845,
    "nu_reactor_dm2_ee": 2.500e-3,
    "nu_acc_theta23": 0.56,
    "nu_acc_dm2_32": 2.50e-3,
    "nu_acc_delta_cp": -1.57,
    "nu_atm_theta23": 0.56,
    "nu_atm_dm2_32": 2.50e-3,
}

COLLIDER_REF_PT = 450.0
COLLIDER_SM_EXP = 1.15

ROOT = Path(__file__).resolve().parents[2]
MUON_CONST_PATH = ROOT / "tools" / "predictors" / "muon_gm2_constants.json"


def muon_sm_prediction() -> float:
    ...
    return total_1e11 * 1e-11


def resolve_sm_baseline(row: dict, obs: str, idx: int) -> float:
    if obs == "muon_g_minus_2":
        return muon_sm_prediction()
    if obs in SM_BASELINES:
        return float(SM_BASELINES[obs])
    if obs == "collider_high_pt_tail":
        ...
        return (COLLIDER_REF_PT / x) ** COLLIDER_SM_EXP
    raise SystemExit(f"row {idx}: unsupported observable_id for SM baseline: {obs}")


def main() -> None:
    args = build_parser("Micro SM predictor (submission-candidate)").parse_args()
    ...
    for i, row in enumerate(rows, start=1):
        if row["domain"] != "micro":
            raise SystemExit(f"row {i}: micro_sm_predict expects domain=micro")
        obs = str(row["observable_id"])
        pred_value = resolve_sm_baseline(row, obs, i)
