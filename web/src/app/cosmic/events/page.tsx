...
export default async function CosmicEventsPage() {
  ...
  return (
    <section className="space-y-5">
      <header className="panel p-5">
        <p className="text-xs uppercase tracking-[0.2em] text-cyan-300">Cosmic Events</p>
        <h1 className="mt-2 text-2xl font-semibold">거시 이벤트별 판정 상세</h1>
      </header>
            ...
            {sorted.map((r) => {
              ...
              return (
                <tr key={`${r.prediction_id}-${r.event_id}`} className="border-t border-slate-800">
                  ...
                  <td className="py-2">{r.standard_error != null ? r.standard_error.toFixed(4) : "-"}</td>
                  <td className="py-2">{r.salt_error != null ? r.salt_error.toFixed(4) : "-"}</td>
                  <td className="py-2">{winner === "STANDARD" ? "LambdaCDM" : winner}</td>
