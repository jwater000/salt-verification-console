...
| 항목 | 현재 판정 | 비고 |
| --- | --- | --- |
| 로컬 운영 설정 `.env.local`에 `DATABASE_URL`이 설정되어 있다 | `YES` | `2026-04-06` 세션에서 값 존재 확인 및 Neon 연결 확인 |
| 로컬 운영 설정 `.env.local`에 `AUTH_SECRET`이 설정되어 있다 | `YES` | `2026-04-06` 세션에서 값 존재 확인 |
...
| runtime check 스크립트가 있다 | `YES` | `scripts/check-runtime.mjs` 존재 |
...
| 게시판 작성 API가 존재한다 | `YES` | `/api/board-posts` 라우트 존재 |
| 게시판 moderation API가 존재한다 | `YES` | `/api/board-posts/[id]/hide`, `/delete` 라우트 존재 |
| `.env.example`가 있다 | `YES` | `web/.env.example` 추가 |
...
- OAuth 로그인, 게시판 글 작성, 게시판 hide/delete도 최소 1회씩 실제로 검증한다.
